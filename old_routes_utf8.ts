...
export async function registerRoutes(
  ...
  // Day boundary is 04:00 AM Jakarta ├óΓé¼ΓÇ¥ before 04:00 counts as previous day
  ...
  app.post(api.attendance.permit.path, upload.single('photo'), async (req, res) => {
    ...
    if (activeSession) {
      // Employee is currently working (or on break) ├óΓé¼ΓÇ¥ close the session
    ...
    // No active session ├óΓé¼ΓÇ¥ permit submitted before starting work
    ...
    const attendance = await storage.createAttendance({
      ...
      checkInPhoto: photoFileId,
      checkIn: now,
      checkOut: now, // immediately closed ├óΓé¼ΓÇ¥ no actual work done
