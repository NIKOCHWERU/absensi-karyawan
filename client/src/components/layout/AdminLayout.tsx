...
function AdminSidebarHeader() {
    ...
    return (
        <div className="flex items-center gap-3 px-4 py-3 border-b border-sidebar-border bg-white min-h-[4rem]">
            <img src="/logo_elok_buah.jpg" alt="Logo" className="w-9 h-9 object-contain rounded-lg shadow-xs shrink-0" />
            {state === "expanded" && (
                <div className="flex flex-col min-w-0 transition-opacity duration-200">
                    <h1 className="text-sm font-black text-gray-900 tracking-tight leading-tight truncate">
                        PT ELOK JAYA ABADHI
                    </h1>
                    <p className="text-[10px] text-green-600 font-bold tracking-wider leading-none">
                        ADMIN PANEL
                    </p>
                </div>
            )}
            ...
            <SidebarMenuItem>
                <DropdownMenu>
                    <DropdownMenuTrigger asChild>
                        ...
                        >
                            <div className="w-8 h-8 rounded-lg bg-gradient-to-br from-green-500 to-emerald-600 text-white flex items-center justify-center font-bold text-sm shadow-xs shrink-0 select-none">
                                {userInitial}
                            </div>
                            {state === "expanded" && (
                    ...
                    >
                        <DropdownMenuLabel className="p-0 font-normal">
                            <div className="flex items-center gap-2 px-4 py-2 text-start text-sm border-b border-gray-50">
                                <div className="w-8 h-8 rounded-lg bg-gradient-to-br from-green-500 to-emerald-600 text-white flex items-center justify-center font-bold text-sm shrink-0">
                                    {userInitial}
                                </div>
                                <div className="grid flex-1 text-start text-sm leading-tight min-w-0">
                                    <span className="truncate font-semibold text-gray-800">
        ...
        <SidebarMenuItem>
            ...
            >
                ...
                {item.badge !== undefined && item.badge > 0 && (
                    <span className="ml-auto bg-green-600 text-white text-[10px] px-2 py-0.5 rounded-full font-bold">
                        {item.badge}
                    </span>
                )}
            </SidebarMenuButton>
            ...
            <SidebarMenuItem>
                <CollapsibleTrigger asChild>
                    ...
                    >
                        ...
                        <span>{item.title}</span>
                        {item.badge !== undefined && item.badge > 0 && (
                            <span className="ml-auto mr-1 bg-green-600 text-white text-[10px] px-2 py-0.5 rounded-full font-bold">
                                {item.badge}
                            </span>
                        )}
            ...
            <DropdownMenu>
                <DropdownMenuTrigger asChild>
                    ...
                    >
                        ...
                        <span>{item.title}</span>
                        {item.badge !== undefined && item.badge > 0 && (
                            <span className="absolute top-1.5 right-1.5 flex h-2 w-2 rounded-full bg-green-600 ring-2 ring-white animate-pulse" />
                        )}
                    </SidebarMenuButton>
                </DropdownMenuTrigger>
                    ...
                    {item.items.map((subItem) => {
                        ...
                        return (
                            <DropdownMenuItem
                                key={subItem.title}
                                onClick={() => setLocation(subItem.url)}
                                className={`flex items-center gap-2 cursor-pointer ${isSubActive ? "bg-green-50 text-green-700 font-semibold" : "text-gray-700 hover:bg-gray-50"}`}
                            >
                                {subItem.icon && <subItem.icon className="h-4 w-4 shrink-0" />}
                                <span>{subItem.title}</span>
                                {subItem.badge !== undefined && subItem.badge > 0 && (
                    ...
                    items: [
                        {
                            ...
                            url: "/admin/mutation-management",
                            icon: UserCog,
                        },
                        {
                            title: "Riwayat Karir",
                            url: "/admin/mutation-history",
                            icon: History,
                        },
                    ]
                }
